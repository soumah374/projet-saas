  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
      ...
      <Card className="p-6 bg-gradient-to-br from-green-50 to-green-100 border-green-200">
        <div className="flex items-center justify-between">
          <div>
            <p className="text-sm font-medium text-green-700">En cours</p>
            <p className="text-3xl font-bold text-green-900">{stats.inProgress}</p>
            <div className="flex items-center gap-1 mt-2">
              <span className="text-sm text-green-600">Avancement moyen: {stats.avgProgress}%</span>
            </div>
          </div>
      ...
      <Card className="p-6 bg-gradient-to-br from-orange-50 to-orange-100 border-orange-200">
        <div className="flex items-center justify-between">
          <div>
            <p className="text-sm font-medium text-orange-700">Échéances urgentes</p>
            <p className="text-3xl font-bold text-orange-900">{stats.urgent}</p>
            <div className="flex items-center gap-1 mt-2">
              <AlertTriangle className="h-4 w-4 text-orange-600" />
              <span className="text-sm text-orange-600">&lt; 7 jours</span>
            </div>
          </div>
      ...
      <Card className="p-6 bg-gradient-to-br from-purple-50 to-purple-100 border-purple-200">
        <div className="flex items-center justify-between">
          <div>
            <p className="text-sm font-medium text-purple-700">Types de projets</p>
            <div className="mt-2 space-y-1">
              {Object.entries(typeStats).map(([type, count]) => (
                <div key={type} className="flex justify-between items-center">
                  <span className="text-xs text-purple-600">{type}</span>
                  <Badge variant="secondary" className="text-xs bg-purple-200">
                    {count}
                  </Badge>
                </div>
              ))}
