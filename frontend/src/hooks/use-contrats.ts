...
export const useEnvoyerContratPDF = () => {
  const queryClient = useQueryClient();
  
  return useMutation({
    mutationFn: ({ id, pdfData }: { id: number; pdfData: string }) => 
      contratsAPI.envoyerContratPDF(id, pdfData),
    onSuccess: (_, { id }) => {
      queryClient.invalidateQueries({ queryKey: ['contrats'] });
      queryClient.invalidateQueries({ queryKey: ['contrat', id] });
      toast.success('Contrat envoyé par email avec succès');
    },
    onError: (error) => {
...
// Types pour les templates de contrat
export interface TemplateContrat {
  id: number;
  nom: string;
  type_template: 'prestation' | 'maintenance' | 'formation' | 'conseil' | 'personnalise';
  type_template_display: string;
  description: string;
  contenu: string;
...
// Hooks pour les templates de contrat
export const useTemplatesContrat = (params?: {
  ...
  return useQuery({
    queryKey: ['templates-contrat', params],
    queryFn: async () => {
      const response = await contratsAPI.getTemplatesContrat(params);
      return response.data;
    },
  });
};

export const useTemplateContratById = (id: number) => {
  return useQuery({
    queryKey: ['template-contrat', id],
    queryFn: async () => {
      const response = await contratsAPI.getTemplateContratById(id);
      return response.data;
    },
    enabled: !!id,
  });
...
export const useCreateTemplateContrat = () => {
  const queryClient = useQueryClient();
  ...
  return useMutation({
    mutationFn: (data: {
      ...
      est_actif?: boolean;
      est_public?: boolean;
    }) => contratsAPI.createTemplateContrat(data),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['templates-contrat'] });
...
export const useUpdateTemplateContrat = () => {
  const queryClient = useQueryClient();
  
  return useMutation({
    mutationFn: ({ id, data }: { id: number; data: any }) => contratsAPI.updateTemplateContrat(id, data),
    onSuccess: (_, { id }) => {
      queryClient.invalidateQueries({ queryKey: ['templates-contrat'] });
      queryClient.invalidateQueries({ queryKey: ['template-contrat', id] });
...
export const useDeleteTemplateContrat = () => {
  const queryClient = useQueryClient();
  
  return useMutation({
    mutationFn: (id: number) => contratsAPI.deleteTemplateContrat(id),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['templates-contrat'] });
...
export const useGenererContratFromTemplate = () => {
  return useMutation({
    mutationFn: (data: {
      template_id: number;
      variables: any;
      contrat_id: number;
    }) => contratsAPI.genererContratFromTemplate(data),
    onError: (error) => {
...
export const useTemplatesActifs = () => {
  return useQuery({
    queryKey: ['templates-actifs'],
    queryFn: async () => {
      const response = await contratsAPI.getTemplatesActifs();
      return response.data;
    },
  });
};

export const useTypesTemplates = () => {
  return useQuery({
    queryKey: ['types-templates'],
    queryFn: async () => {
      const response = await contratsAPI.getTypesTemplates();
      return response.data;
    },
  });
}; 