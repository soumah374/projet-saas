# Template par défaut (HTML)
def template_html_contrat(variables):
  return f"""<h3 style="color: #000000; font-size: 20px; font-weight: bold; margin: 25px 0 15px 0; border-bottom: 2px solid #000000; padding-bottom: 8px; font-family: Arial, Helvetica, sans-serif;">CONTRAT DE PRESTATION DE SERVICES</h3>
...
<strong style="font-weight: bold; color: #000000; font-family: Arial, Helvetica, sans-serif;">{variables.get('RAISON_SOCIALE_PRESTATAIRE', 'SAKOM SARL')}</strong>,<br>
Société {variables.get('FORME_JURIDIQUE', 'SARL')} au capital de {variables.get('MONTANT_CAPITAL', '100,000,000')} GNF,<br>
immatriculée au RCS de {variables.get('VILLE_RCS', 'Conakry')} sous le numéro {variables.get('SIRET', 'GN12345678901234')},<br>
dont le siège social est situé à {variables.get('ADRESSE_PRESTATAIRE', '123 Avenue de la République, Conakry, Guinée')},<br>
représentée par {variables.get('NOM_REPRESENTANT', 'Directeur Général')}, en sa qualité de {variables.get('FONCTION_REPRESENTANT', 'Directeur Général')},
...
<p style="margin: 15px 0; text-align: justify; font-family: Arial, Helvetica, sans-serif; line-height: 1.7; color: #000000;">
<strong style="font-weight: bold; color: #000000; font-family: Arial, Helvetica, sans-serif;">{variables.get('NOM_CLIENT', 'Client')}</strong>,<br>
{variables.get('TYPE_CLIENT', 'Société')} domicilié(e) à {variables.get('ADRESSE_CLIENT', 'Adresse du client')},<br>
immatriculé(e) sous le numéro {variables.get('NUMERO_IDENTIFICATION', 'N/A')},<br>
représenté(e) par {variables.get('NOM_REPRESENTANT_CLIENT', 'Représentant')}, en sa qualité de {variables.get('FONCTION_REPRESENTANT_CLIENT', 'Représentant')},<br>
...
<p style="margin: 15px 0; text-align: justify; font-family: Arial, Helvetica, sans-serif; line-height: 1.7; color: #000000;">
Le présent contrat a pour objet la réalisation des prestations définies dans le <strong style="font-weight: bold; color: #000000; font-family: Arial, Helvetica, sans-serif;">devis n° {variables.get('NUMERO_DEVIS', 'N/A')}</strong> daté du {variables.get('DATE_DEVIS', 'N/A')}, annexé au présent contrat et accepté par le Client.
</p>
...
<p style="margin: 15px 0; text-align: justify; font-family: Arial, Helvetica, sans-serif; line-height: 1.7; color: #000000;">
Le présent contrat prend effet à compter de sa date de signature pour une durée estimée de {variables.get('DUREE_ESTIMEE', 'À définir')} à compter du début des travaux fixé au {variables.get('DATE_DEBUT_PRESTATION', 'N/A')}.
</p>
...
<strong style="font-weight: bold; color: #000000; font-family: Arial, Helvetica, sans-serif;">{variables.get('DESCRIPTION_PRESTATION', 'Prestations définies dans le devis')}</strong><br>
...
<p style="margin: 15px 0; text-align: justify; font-family: Arial, Helvetica, sans-serif; line-height: 1.7; color: #000000;">
Le montant total de la prestation est fixé à <strong style="font-weight: bold; color: #000000; font-family: Arial, Helvetica, sans-serif;">{variables.get('MONTANT_TTC', '0')} GNF TTC</strong>, selon le devis accepté.<br>
Modalités de paiement :
</p>

<ul style="margin: 15px 0; padding-left: 20px; font-family: Arial, Helvetica, sans-serif; line-height: 1.7; color: #000000;">
<li style="margin: 5px 0;">{variables.get('MODALITES_PAIEMENT', '30% à la commande, 70% à la livraison')}</li>
...
<p style="margin: 15px 0; text-align: justify; font-family: Arial, Helvetica, sans-serif; line-height: 1.7; color: #000000;">
En cas de manquement grave de l'une des parties à ses obligations contractuelles, le contrat pourra être résilié de plein droit après mise en demeure restée sans effet pendant {variables.get('DELAI_RESILIATION', '30')} jours.
</p>
...
<p style="margin: 15px 0; text-align: justify; font-family: Arial, Helvetica, sans-serif; line-height: 1.7; color: #000000;">
Fait à {variables.get('VILLE_SIGNATURE', 'Conakry')}, le {variables.get('DATE_SIGNATURE', 'N/A')},<br>
En deux exemplaires originaux.
</p>

<table style="width: 100%; margin: 30px 0; border-collapse: collapse; font-family: Arial, Helvetica, sans-serif;">
<tr>
<td style="width: 50%; padding: 20px; text-align: center; border: 1px solid #000000;">
